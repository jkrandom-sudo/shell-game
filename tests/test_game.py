...
class TestGame(unittest.TestCase):
    ...
    def test_play_round_correct_wrong_and_invalid(self):
        settings = base_settings()
        starts = [0, 1, 2, 0, 1]
        swaps = [[(0, 1)], [(1, 2)], [(0, 1)], [(0, 2)], [(1, 2)]]
        with mock.patch.object(game.core, "new_round", side_effect=starts), mock.patch.object(game.core, "make_swaps", side_effect=swaps), mock.patch("builtins.input", StackedInput(["2", "1", "x", "3", "3"])):
    ...
    def test_play_round_quits(self):
        settings = base_settings()
        with mock.patch.object(game.core, "new_round", return_value=0), mock.patch.object(game.core, "make_swaps", return_value=[(0, 1)]), mock.patch("builtins.input", StackedInput(["q"])):
